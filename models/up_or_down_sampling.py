...
def upfirdn2d_torch(x, f, up=1, down=1, pad=0, flip_filter=False, gain=1):
    ...
    f1 = torch.ones([1], dtype=torch.float32, device=x.device) * f[0]
    f2 = torch.ones([1], dtype=torch.float32, device=x.device) * f[1]
    if len(f) == 4:
      f3 = torch.ones([1], dtype=torch.float32, device=x.device) * f[2]
      f4 = torch.ones([1], dtype=torch.float32, device=x.device) * f[3]
    else:
      f3 = 0.0
      f4 = 0.0
...
def upsample_2d(x, k=None, factor=2, gain=1, use_torch=USE_TORCH_IMPLEMENTATION):
...
def downsample_2d(x, k=None, factor=2, gain=1, use_torch=USE_TORCH_IMPLEMENTATION):
