"""
Implements base diffusion class. Simplification of https://github.com/sillsill777/Diffusion.
"""
...
class GaussianDiffusion(nn.Module):
    def __init__(self, 
        ...
        # the backward process (derived with Bayes), beta tilde is the variance
        ...
        self.register_buffer(
            "sqrt_recip_alpha_bar_min_1", torch.sqrt(1.0 / alpha_bar - 1)
        )
    ...
    ##### Forward Process / Denoising Process ####################################
    ...
    def p_sample(self, xt, t, clip=True):
        ...
        """
        batched_time = torch.full(
            (xt.shape[0],), t, device=self.device, dtype=torch.long
        )
        pred_noise = self.unet(xt, batched_time)  # corresponds to epsilon_{theta}
        if clip:
            x0 = (
                self.sqrt_recip_alpha_bar[t] * xt
                - self.sqrt_recip_alpha_bar_min_1[t] * pred_noise
            )
            x0.clamp_(-1.0, 1.0)
    ...
    @torch.inference_mode()
    def sample(
        self, batch_size=16, return_all_timestep=False, clip=True, min1to1=False
    ):
        """
        ...
        """
        xT = torch.randn(
            [batch_size, self.channel, self.image_size, self.image_size],
            device=self.device,
        )
        ...
        images = (
            xt
            if not return_all_timestep
            else torch.stack(denoised_intermediates, dim=1)
        )
...
class DDIM_Sampler(nn.Module):
    def __init__(
        ...
        assert clip in [
            ...
            False,
            "both",
        ], "clip must be one of [True, False, 'both']"
        if self.save:
            assert (
                self.calculate_fid is True
            ), "To save model based on FID score, you must set [calculate_fid] to True"
        ...
        self.register_buffer(
            "tau",
            torch.linspace(
                -1, ddpm_steps - 1, steps=self.ddim_steps + 1, dtype=torch.long
            )[1:],
        )

        alpha_tau_i = alpha_bar[self.tau]
        alpha_tau_i_min_1 = F.pad(
            alpha_bar[self.tau[:-1]], pad=(1, 0), value=1.0
        )  # alpha_0 = 1
        ...
        assert (
            self.coeff[0] == 0.0 and self.sqrt_alpha_i_min_1[0] == 1.0
        ), "DDIM parameter error"
    ...
    def ddim_p_sample(self, model, xt, i, clip=True):
        ...
        x0 = (
            model.sqrt_recip_alpha_bar[t] * xt
            - model.sqrt_recip_alpha_bar_min_1[t] * pred_noise
        )
        if clip:
            x0.clamp_(-1.0, 1.0)
            pred_noise = (
                model.sqrt_recip_alpha_bar[t] * xt - x0
            ) / model.sqrt_recip_alpha_bar_min_1[t]
    ...
    def sample(
        ...
        xT = (
            ...
            if noise is None
            else noise.to(self.device)
        )
        denoised_intermediates = [xT]
        xt = xT
        for i in tqdm(
            reversed(range(0, self.ddim_steps)),
            desc="DDIM Sampling",
            total=self.ddim_steps,
            leave=False,
        ):
