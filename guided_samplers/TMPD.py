...
# this has OT in its name as it requires the copmutation of the derivative
# @title PiGDM guidance
def tmpd_ot_guidance(
    model_fn, x_t, t, H_func, y_obs, sigma_y, alpha_t, std_t, device="cuda", **kwargs
):
    """
    TMPD guidance for OT path.
    Returns ∇ log p(y|x_t) approximation.
    """
    t_batched = torch.ones(x_t.shape[0], device=device) * t

    # compute r_t^2
    r_t_2 = std_t**2 / (std_t**2 + alpha_t**2)
    ...
    # clip
    clipped_vec = (gamma_t * scaled_grad).clamp(-1, 1) + (flow_pred)
