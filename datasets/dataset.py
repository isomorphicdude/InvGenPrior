...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
    ...
    if config.data.dataset == "CIFAR10":
        ...
        dataset_builder = tfds.builder(
            f"lsun/{config.data.category}", data_dir=config.data.root_path
        )
        train_split_name = "train"
        eval_split_name = "validation"
        ...
        dataset_builder = tf.data.TFRecordDataset(
            config.data.tfrecords_path, data_dir=config.data.root_path
        )
...
def get_pytorch_dataset(config):
    ...
    if config.data.dataset == "CelebA-HQ-Pytorch":
        ...
        return tds.celeba_hq_dataset(
            config.training.data_dir, config.training.batch_size, transform
        ), tds.celeba_hq_dataset(
            config.training.data_dir, config.training.batch_size, transform
        )
    elif config.data.dataset == "AFHQ-CAT-Pytorch":
        transform = tr.Resize(256)
        return tds.afhq_dataset(
            config.training.data_dir, config.training.batch_size, "cat", transform
        ), tds.afhq_dataset(
            config.training.data_dir, config.training.batch_size, "cat", transform
        )
    else:
